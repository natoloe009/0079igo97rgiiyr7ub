...
function OrionLib:MakeWindow(WindowConfig)
    ...
    -- Works for PC (mouse) + Mobile (touch)
    ...
    -- Create dropdown UI elements (same as before)
    local DropdownList = MakeElement("List")
    ...
    local function updateSelectedText()
        ...
        elseif #Dropdown.Value == 1 then
            ...
            DropdownFrame.F.Selected.Text = #Dropdown.Value .. " selected"
        end
    end
    ...
    local function AddOptions(Options)
        for _, Option in pairs(Options) do
            local OptionBtn = AddThemeObject(SetProps(SetChildren(MakeElement("Button", Color3.fromRGB(40, 40, 40)), {
                ...
                -- Add a checkmark indicator for selected items
                AddThemeObject(SetProps(MakeElement("Image", "rbxassetid://7072718332"), {
    ...
    function Dropdown:Set(Values)
        ...
        for _, Value in pairs(Values) do
            if table.find(Dropdown.Options, Value) and Dropdown.Buttons[Value] then
                table.insert(Dropdown.Value, Value)
                TweenService:Create(Dropdown.Buttons[Value], TweenInfo.new(.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = 0}):Play()
                TweenService:Create(Dropdown.Buttons[Value].Title, TweenInfo.new(.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextTransparency = 0}):Play()
                Dropdown.Buttons[Value].Checkmark.Visible = true
            end
        end
    ...
    -- Make selection indicators larger for touch devices
    local ColorSelection = Create("ImageLabel", {
        Size = UDim2.new(0, 24, 0, 24), -- Increased size for touch
    ...
    -- Make color area larger for touch
    local Color = Create("ImageLabel", {
        Size = UDim2.new(1, -40, 1, 0), -- Increased size
        Visible = false,
    ...
    -- Make hue area wider for touch
    local Hue = Create("Frame", {
        Size = UDim2.new(0, 30, 1, 0), -- Increased width
        Position = UDim2.new(1, -30, 0, 0), -- Adjusted position
        Visible = false,
        ZIndex = 2
    }, {
    ...
    local ColorTouchArea = Create("TextButton", {
        Size = UDim2.new(1, 0, 1, 0),
        BackgroundTransparency = 1,
        Text = "",
        ZIndex = 3,
    ...
    local HueTouchArea = Create("TextButton", {
        Size = UDim2.new(1, 0, 1, 0),
        BackgroundTransparency = 1,
        Text = "",
        ZIndex = 3,
    ...
    local ColorpickerContainer = Create("Frame", {
        ...
        Hue,
        Color,
        ColorTouchArea, -- Add touch area for color
        HueTouchArea,   -- Add touch area for hue
        Create("UIPadding", {
    ...
    local ColorpickerBox = AddThemeObject(SetChildren(SetProps(MakeElement("RoundFrame", Color3.fromRGB(255, 255, 255), 0, 4), {
        Size = UDim2.new(0, 28, 0, 28), -- Slightly larger for touch
        Position = UDim2.new(1, -12, 0.5, 0),
        AnchorPoint = Vector2.new(1, 0.5)
    }), {
        AddThemeObject(MakeElement("Stroke"), "Stroke")
    ...
    -- Make colorpicker frame taller when expanded for mobile
    ...
    AddConnection(Click.MouseButton1Click, function()
        ...
        -- Make expanded size larger for mobile
        TweenService:Create(ColorpickerFrame,TweenInfo.new(.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),{Size = Colorpicker.Toggled and UDim2.new(1, 0, 0, 160) or UDim2.new(1, 0, 0, 38)}):Play()
    ...
    local function handleColorInput(input)
        ...
        local isMouseButton1 = input.UserInputType == Enum.UserInputType.MouseButton1
        
        if isTouch or isMouseButton1 then
            if ColorInput then
                ColorInput:Disconnect()
            end
            
            local connection
            connection = AddConnection(RunService.Heartbeat:Connect(function()
                if not ColorTouchArea.Active then return end
                
                -- Get input position based on device type
                local inputPosition
                if isTouch then
                ...
                local ColorX = (math.clamp(inputPosition.X - Color.AbsolutePosition.X, 0, Color.AbsoluteSize.X) / Color.AbsoluteSize.X)
                local ColorY = (math.clamp(inputPosition.Y - Color.AbsolutePosition.Y, 0, Color.AbsoluteSize.Y) / Color.AbsoluteSize.Y)
    ...
    -- Function to handle hue input for both mouse and touch
    local function handleHueInput(input)
        ...
        if isTouch or isMouseButton1 then
            ...
            local connection
            connection = AddConnection(RunService.Heartbeat:Connect(function()
                if not HueTouchArea.Active then return end
                ...
                local HueY = (math.clamp(inputPosition.Y - Hue.AbsolutePosition.Y, 0, Hue.AbsoluteSize.Y) / Hue.AbsoluteSize.Y)
    ...
    AddConnection(ColorTouchArea.InputBegan, handleColorInput)
    AddConnection(HueTouchArea.InputBegan, handleHueInput)
    
    -- Also keep the original connections for mouse input on the visible elements
    AddConnection(Color.InputBegan, handleColorInput)
    AddConnection(Hue.InputBegan, handleHueInput)

    -- Handle input ending for both mouse and touch
    ...
    AddConnection(ColorTouchArea.InputEnded, handleInputEnded)
    AddConnection(HueTouchArea.InputEnded, handleInputEnded)
    AddConnection(Color.InputEnded, handleInputEnded)
    AddConnection(Hue.InputEnded, handleInputEnded)
    ...
    function Colorpicker:Set(Value)
        ...
        -- Update the selectors to match the new color
